...
- **Tailwind v4 `@theme` block** — derived **deterministically** from the
  styleguide + brand in `agent/lib/derive-tokens.ts`.
...
```bash
npm install eve context.dev ai @ai-sdk/anthropic
```
...
<ComponentSource
  src="registry/eve/extract-design-md/agent/tools/compose_design_md.ts"
  title="agent/tools/compose_design_md.ts"
/>

<ComponentSource
  src="registry/eve/extract-design-md/agent/lib/compose.ts"
  title="agent/lib/compose.ts"
/>

<ComponentSource
  src="registry/eve/extract-design-md/agent/lib/design-md.ts"
  title="agent/lib/design-md.ts"
/>

<ComponentSource
  src="registry/eve/extract-design-md/agent/lib/derive-tokens.ts"
  title="agent/lib/derive-tokens.ts"
/>
...
- **Reshape the document.** Edit `agent/lib/design-md.ts` — the prompt spec is
  the single source of truth for the frontmatter keys, token names, and sections.
- **Swap the model.** Change the model in `agent/lib/compose.ts` — it uses
  `claude-sonnet-4-6` (vision-capable). The reference uses `gpt-5.4-mini`; any
  multimodal model that can read the screenshot works.
- **Temperature.** Adjust `temperature` in `agent/lib/compose.ts` (the reference
