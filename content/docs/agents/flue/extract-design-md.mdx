...
Given a domain, it gathers the styleguide, brand, a homepage screenshot, and
clean page Markdown through context.dev, then returns **three outputs**:
...
```bash
npm install @flue/runtime context.dev ai @ai-sdk/anthropic
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource
  src="registry/flue/extract-design-md/workflows/extract-design-md.ts"
  title="workflows/extract-design-md.ts"
/>

<ComponentSource
  src="registry/flue/extract-design-md/lib/compose.ts"
  title="lib/compose.ts"
/>

<ComponentSource
  src="registry/flue/extract-design-md/lib/design-md.ts"
  title="lib/design-md.ts"
/>

<ComponentSource
  src="registry/flue/extract-design-md/lib/derive-tokens.ts"
...
- **Reshape the document.** Edit `lib/design-md.ts` — the prompt spec is the
  single source of truth for the frontmatter keys, token names, and sections.
...
- **Temperature.** Adjust `temperature` in `lib/compose.ts` (the reference uses
  `0.2`) to trade determinism for variety.
