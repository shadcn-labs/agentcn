...
const referenceImageSchema = v.union([
  v.object({
    url: v.pipe(
      v.string(),
      v.url(),
      v.check((val) => isHttpUrl(val), 'Reference image URL must use http or https.'),
    ),
  }),
  v.object({
    base64: v.pipe(
      v.string(),
      v.minLength(1),
      v.check((val) => isBase64Payload(val), 'Reference image must be a standard base64 payload.'),
    ),
  }),
])
...
const inputSchema = v.object({
  ...
  assetType: v.pipe(v.string(), v.minLength(1)),
  brief: v.pipe(v.string(), v.minLength(20)),
  dimensions: v.pipe(v.string(), v.minLength(1)),
  referenceImages: v.optional(v.array(referenceImageSchema)),
})

type GenerateSvgInput = v.InferOutput<typeof inputSchema>
type ReferenceImageInput = v.InferOutput<typeof referenceImageSchema>
...
export const generateSvgWithArrow = defineTool({
  ...
  return addIconRootDefaults(
    addStrokeToUnstyledIconPaths(
      svg
        ...
        .replace(/\bfill\s*:\s*(#[0-9a-f]{3,8})\b/giu, (_match: string, color: string) =>
          isAccentColor(color) ? `fill:${color}` : 'fill:none',
        )
        .replace(/\bfill\s*=\s*(["'])(#[0-9a-f]{3,8})\b\1/giu, (_match: string, quote: string, color: string) =>
...
function isIconAsset(input: v.InferOutput<typeof inputSchema>): boolean {
  const targetViewBox = parseTargetViewBox(input.dimensions)
...
function svgDescription(input: v.InferOutput<typeof inputSchema>): string {
