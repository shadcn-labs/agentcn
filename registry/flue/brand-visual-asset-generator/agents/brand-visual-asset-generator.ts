...
import { generateSvgWithArrow } from '../tools/generate-svg-with-arrow.ts'
import { contextDevSearch } from '../tools/context-dev-search.ts'
import brandVisualAssets from '../skills/brand-visual-assets/SKILL.md' with { type: 'skill' }
...
3. Use the \`context_dev_search\` tool to discover brand data from Context.dev.
   Use \`search_docs\` when you need exact SDK method or parameter names, then
   use \`execute\` to gather source data.
4. Through Context.dev, retrieve at minimum:
...
9. If the Context.dev call fails because the API key is missing, invalid,
   rate-limited, or unavailable, stop and report the configuration or API failure.
   Do not fabricate brand facts.
...
- Do not use a language-model subagent for \`quiverai/arrow-1.1\`; it is an image
  ...
  `,
  skills: [brandVisualAssets],
  tools: [generateSvgWithArrow, contextDevSearch],
}))
